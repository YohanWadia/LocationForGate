...
class _ListenLocationState extends State<ListenLocationWidget> {
  ...
  doit() async {
    await player.setAsset('assets/audio/opening.mp3');
    player.play();
  }
  ...
  Future<int> calcDistance()  async { //(home , newLocation). casted to int
    ...
    if(_location!=null && _location?.longitude != null) {
      ...
      else if (d < 160 && d > 100 && wasEver1000plus == true && wasEverPlayed == false) {
        await player.setAsset('assets/audio/opening.mp3');
        player.play();
        wasEverPlayed = true;
      }
      else if (d < 60) { wasEver1000plus = false;  wasEverPlayed = false; }
    }
    print("returned $d");
  return d;
  }
